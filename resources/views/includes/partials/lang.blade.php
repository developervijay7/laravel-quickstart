@if(config('quickstart.locale.status') && count(config('quickstart.locale.languages')) > 1)
    <span x-data="{'showLang': false}" title="{{ __('labels.lang-switch') }}">
        <span @click="showLang = !showLang" @click.away="showLang = false" class="flex items-center">
            <x-utils.link :text="__(getLocaleName(app()->getLocale()))" id="langPicker" aria-expanded="true" aria-haspopup="true">
            <x-slot name="icon">
                <span
