<div x-cloak class="flex items-center">
    <button x-show="currentTheme === 'dark'" @click="currentTheme = 'system'" title="{{ __('labels.dark-mode') }}">
        <x-icons.moon
            class="p-1 w-8 h-8 text-gray-100 bg-gray-900 rounded-md transition cursor-pointer dark:hover:bg-gray-600"/>
    </button>

    <button x-show="currentTheme === 'light'" @click="currentTheme = 'dark'" title="{{ __('labels.light-mode') }}">
        <x-icons.sun
